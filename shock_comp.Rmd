...
function(resids, lags.pt, p_plus_q) 
{
    nobs <- nrow(resids)
    nvar <- ncol(resids)
    ...
    PVAL <- pchisq(Qh.star, df = PARAMETER, lower.tail = TRUE)
    return(result)
}
...
svarma_data <- fred_md %>% dplyr::select(date, DLIP, DLCPI, EBP, FEDFUNDS) %>%
